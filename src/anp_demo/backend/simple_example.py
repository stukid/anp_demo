...
from .log_base import set_log_color_level
from .config import (
    DASHSCOPE_API_KEY,
    DASHSCOPE_BASE_URL,
...
You are a general-purpose intelligent network data exploration tool.
 Your goal is to find the information and APIs that users need by recursively accessing various data formats (including JSON-LD, YAML, etc.) to complete specific tasks.
...
async def handle_tool_call(
    ...
    if function_name == "anp_tool":
        ...
        try:
            ...
            event_queue.append(f"crawled {url}")
            messages.append(
                {
                    "role": "tool",
...
async def simple_crawl(
    ...
    try:
        event_queue.append(f"begin to crawl with anp_tool: {initial_url}")
    ...
    while current_iteration < max_documents:
        ...
        event_queue.append(f"Model response: {response_message.content}")
        ...
        for tool_call in response_message.tool_calls:
            event_queue.append(f"handling tool call from the response: {tool_call}")
            await handle_tool_call(
                tool_call, messages, anp_tool, crawled_documents, visited_urls, event_queue
            )
        ...
        if (
            ...
            event_queue.append(f"Reached the maximum number of documents to crawl {max_documents}, making final summary")
            continue
